...
import type { AnalyzedFile, DocumentationFile, FileDependency, NormalizedSymbol } from "../../types/normalizedAnalysis.js";
...
const DOC_NAME_PRIORITY = ["brief", "spec", "overview", "readme"];
const MAX_DOC_FILES = 5;
const MAX_DOC_CHARS = 8000;

function scoreDocName(filename: string): number {
  const lower = filename.toLowerCase();
  for (let i = 0; i < DOC_NAME_PRIORITY.length; i++) {
    if (lower.includes(DOC_NAME_PRIORITY[i])) {
      return DOC_NAME_PRIORITY.length - i;
    }
  }
  return 0;
}

async function scanDocumentation(rootAbsolutePath: string, repoRoot: string): Promise<DocumentationFile[]> {
  const found: Array<{ absPath: string; score: number }> = [];
  ...
  async function visit(absolutePath: string, depth: number): Promise<void> {
    ...
    if (stat.isDirectory()) {
      ...
      if (IGNORE_DIRECTORIES.has(name)) return;
      const entries = await fs.readdir(absolutePath, { withFileTypes: true });
      for (const entry of entries) {
        await visit(path.join(absolutePath, entry.name), depth + 1);
      }
      return;
    }
    ...
    found.push({ absPath: absolutePath, score: scoreDocName(path.basename(absolutePath)) });
  }

  await visit(rootAbsolutePath, 0);
  found.sort((a, b) => b.score - a.score || a.absPath.localeCompare(b.absPath));

  const result: DocumentationFile[] = [];
  for (const { absPath } of found.slice(0, MAX_DOC_FILES)) {
    const raw = await fs.readFile(absPath, "utf8");
    const truncated = raw.length > MAX_DOC_CHARS;
    result.push({
      path: toRelativePath(repoRoot, absPath),
...
export async function buildAnalysis(targetPath: string): Promise<NormalizedAnalysis> {
  const env = getEnv();
  const repoRootAbsolute = path.resolve(env.AGENT_REPO_ROOT);
  const absoluteTarget = path.isAbsolute(targetPath)
    ? path.resolve(targetPath)
    : path.resolve(repoRootAbsolute, targetPath);
  const relativeToRoot = path.relative(repoRootAbsolute, absoluteTarget);
  const safeTarget = await validatePath(env.AGENT_REPO_ROOT, relativeToRoot);
  const [{ files, truncated }, documentation] = await Promise.all([
    collectFiles(safeTarget, env.AGENT_REPO_ROOT),
    scanDocumentation(safeTarget, env.AGENT_REPO_ROOT),
  ]);
